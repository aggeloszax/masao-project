...
from api.schemas.menu import LanguageCode  # noqa: E402

MENU_JSON = ROOT / "frontend" / "src" / "data" / "menu-mock.json"
OUTPUT_SQL = ROOT / "backend" / "sql" / "002_seed_full_menu_from_frontend.sql"
# Single source of truth: the Pydantic Literal that validates every request.
LANGUAGES: tuple[str, ...] = get_args(LanguageCode)
...
def build_language_migration(menu: list[dict[str, Any]], language: str) -> str:
    """Build a self-contained migration that adds one language.

    Widens the check constraints to the current LanguageCode set and inserts
    the language's translation rows with ON CONFLICT DO NOTHING, so running it
    against a live database never overwrites admin-edited data and never
    requires re-running the full 002 seed.
    """
    if language not in LANGUAGES:
        raise SystemExit(f"Unknown language {language!r}; expected one of {LANGUAGES}")
    allowed = ", ".join(sql_quote(code) for code in LANGUAGES)
    category_translations = [
        row for row in category_translation_rows(menu) if row[1] == language
    ]
    item_translations = [
        row for row in item_translation_rows(menu) if row[1] == language
    ]

    return f"""-- Generated by backend/scripts/generate_menu_seed_sql.py --language {language}.
-- Adds {language!r} to the language constraints and inserts its translation
...
alter table menu_category_translations
    ...
    check (language_code in ({allowed}));
...
def main() -> None:
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == "--language":
        language = sys.argv[2]
        output = ROOT / "backend" / "sql" / sys.argv[3] if len(sys.argv) > 3 else None
        if output is None:
            raise SystemExit("Usage: generate_menu_seed_sql.py --language <code> <output.sql>")
        output.write_text(build_language_migration(menu, language), encoding="utf-8", newline="\n")
