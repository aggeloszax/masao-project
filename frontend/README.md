...
Greek is the base language for menu records. English, German, Italian, Swedish, French, Russian, Hebrew, and Turkish translations live under each item's `translations` object. The language contract is `el`, `en`, `de`, `it`, `sv`, `fr`, `ru`, `he`, and `tr`. Hebrew is right-to-left; the app flips to `dir="rtl"` automatically via `isRtl()` in `src/i18n/config.ts`.
...
- translated `name`, `description`, and `category` for `en`, `de`, `it`, `sv`, `fr`, `ru`, `he`, and `tr`
